...
use MergeInc\Sort\Dependencies\GuzzleHttp\Exception\ConnectException;
use MergeInc\Sort\Dependencies\GuzzleHttp\Exception\RequestException;
use MergeInc\Sort\Dependencies\GuzzleHttp\Promise as P;
use MergeInc\Sort\Dependencies\GuzzleHttp\Promise\FulfilledPromise;
use MergeInc\Sort\Dependencies\GuzzleHttp\Promise\PromiseInterface;
use MergeInc\Sort\Dependencies\GuzzleHttp\Psr7\LazyOpenStream;
use MergeInc\Sort\Dependencies\GuzzleHttp\TransferStats;
use MergeInc\Sort\Dependencies\GuzzleHttp\Utils;
use MergeInc\Sort\Dependencies\Psr\Http\Message\RequestInterface;
use MergeInc\Sort\Dependencies\Psr\Http\Message\UriInterface;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \MergeInc\Sort\Dependencies\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \MergeInc\Sort\Dependencies\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \MergeInc\Sort\Dependencies\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \MergeInc\Sort\Dependencies\GuzzleHttp\Psr7\Utils::streamFor($sink);
