...
class Container implements ContainerInterface, FactoryInterface, InvokerInterface {
	 ...
	 * @var (Definition|null)[]
	 */
	private $fetchedDefinitions = array();
	...
	private function getEntryType( $entry ): string {
		...
		if ( is_array( $entry ) ) {
			return preg_replace( array( '/^array \(/', '/\)$/' ), array( '[', ']' ), var_export( $entry, true ) );
		}
	...
	protected function setDefinition( string $name, Definition $definition ) {
		...
		$this->fetchedDefinitions = array(); // Completely clear this local cache
	...
	private function createDefaultDefinitionSource(): SourceChain {
		$source = new SourceChain( array( new ReflectionBasedAutowiring() ) );
		$source->setMutableDefinitionSource( new DefinitionArray( array(), new ReflectionBasedAutowiring() ) );
