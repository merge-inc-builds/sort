...
class Sort {
	...
	final public function getFromContainer( string $key ) {
		if ( ! ( $this->_container ?? false ) ) {
			...
			$dataHelper          = new DataHelper( $salesEncoder, $mapper );
			$httpClient          = new HttpClient();
			$orderRecorder       = new OrderRecorder( $salesCalculator, $dataHelper );
			$productsHelper      = new ProductsHelper( $dataHelper, $salesCalculator, $mapper );
			...
			$initializeProductMetaKeysOnDuplicationController    =
				new InitializeProductMetaKeysOnDuplicationController( $mapper, $dataHelper );
			$injectAdminJavascriptController                     = new InjectAdminJavascriptController( $environmentDetector, $dataHelper );
			...
			$pageDetectorAndDataInjectionController              =
				new PageDetectorAndDataInjectionController( $environmentDetector, $dataHelper );
			...
			$settingsRegistrationController                      = new SettingsRegistrationController( $engine, $dataHelper, $mapper );
			...
			$this->container[ EnvironmentDetector::class ] = $environmentDetector;
			$this->container[ Mapper::class ]              = $mapper;
